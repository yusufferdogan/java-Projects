 ...
 * set can not exist same element more the once
 * @author Ahmet yusuf erdoğan
 * @version 1.0
 * @since 2020-01-17
 * @param <T> tested as integer and double
 */
public class GtuSet<T>  extends GTUContainer<T> {
     ...
     * @throws IllegalArgumentException if same element exist in the set
     */
    ...
    public void insert(T t) throws IllegalArgumentException
    {
        try {
            if (contains(t)){
                throw new IllegalArgumentException("same element can not added more then once element : " + t);
            }
